...
def readstandard(standardfile):
    ...
    if os.path.isfile(standardfile):
        ...
        listastandard = lickshane.__path__[0] + '/standard/stdlist/' + standardfile
...
def defsex(namefile):
    ...
    sexfile = lickshane.__path__[0] + '/standard/sex/default.sex'
    f = open(sexfile, 'r')
    ss = f.readlines()
    f.close()
    ff = open(namefile, 'w')
    for i in ss:
        if string.count(i, 'PARAMETERS_NAME') == 1:
            ff.write('PARAMETERS_NAME  "' + lickshane.__path__[0] + '/standard/sex/default.param"\n')
        elif string.count(i, 'FILTER_NAME') == 1:
            ff.write('FILTER_NAME  "' + lickshane.__path__[0] + '/standard/sex/default.conv"\n')
        elif string.count(i, 'STARNNW_NAME') == 1:
            ff.write('STARNNW_NAME "' + lickshane.__path__[0] + '/standard/sex/default.nnw"\n')
        else:
            ff.write(i)
    ff.close()
    return namefile
...
def defswarp(namefile, imgname, _combine, gain=''):
    ...
    if _combine.lower() in ['median']:
        ...
        _combine = 'SUM'
    swarpfile = lickshane.__path__[0] + '/standard/sex/default.swarp'
    f = open(swarpfile, 'r')
    ss = f.readlines()
    f.close()
    ff = open(namefile, 'w')
...
def airmass(img, overwrite=True, _observatory='lasilla'):
    import lickshane
    from lickshane.util import readhdr, readkey3, delete
    from pyraf import iraf
    ...
    if readkey3(hdr, 'UTC'):
        ...
        if overwrite and _air < 99.:
            lickshane.util.updateheader(img, 0, {'AIRMASS': [_air, 'mean airmass computed with astcalc']})
    else:
        _air = ''
    return _air
...
def phase3header(img):
    ...
    hedvec0 = {'DATAMIN': [float(min(img_data[isfinite(img_data)])), ''],
               ...
               'PROCSOFT': ['lickshane_' + str(lickshane.__version__), 'pipeline version']}
    ...
    lickshane.util.updateheader(img, 0, hedvec0)
...
def correctobject(img, coordinatefile):
    ...
    import lickshane
    from lickshane.util import readstandard, readhdr, readkey3
    ...
    dd = arccos(
        sin(_dec * scal) * sin(decstd * scal) + cos(_dec * scal) * cos(decstd * scal) * cos((_ra - rastd) * scal)) * (
             (180 / pi) * 3600)
    if min(dd) < 5200:
        lickshane.util.updateheader(img, 0, {'OBJECT': [std[argmin(dd)], 'Original target.']})
...
def archivingtar(outputlist, nametar):
    import os, string, re
    from lickshane.util import delete
...
def StoN(img, ran=50):
    import lickshane
    from lickshane.util import readspectrum
...
def spectraresolution(img):
    ...
    risoluzioni['lickshane']['blu', '2.0'] = 99
    risoluzioni['lickshane']['blu', '1.6'] = 99
    risoluzioni['lickshane']['red', '6.0'] = 99
    risoluzioni['lickshane']['red', '2.0'] = 99
    risoluzioni['lickshane']['red', '1.6'] = 99
...
def classifyfast(fitsfile, program='snid'):
    ...
    if trigger:
        ...
        print '\n##################\n BORING SN ...... \n\n'
    #     print '\n We report that a spectrum of '+fitsfile+' was obtained robotically on Aug XX with the LICKSHANE spectrograph '+
...
def spectraresolution3(img0, ww=25):
    ...
    if len(maxtab) <= 0:      maxtab, mintab = lickshane.util.peakdet(yy, median(yy))
    if len(maxtab) <= 0:
        lines = []
    else:
        lines0, b = zip(*maxtab)
